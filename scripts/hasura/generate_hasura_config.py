...
def _omit_from_configmap(var: str) -> bool:
    """
    Some settings are intentionally configured outside the ConfigMap.

    Redis connection settings are injected via Vault templates in
    `manifests/hasura.yaml` so we omit all *_REDIS_* env vars from the generated
    ConfigMap to prevent confusion and accidental override.
    """

    return "_REDIS_" in var
...
def _render_env(
    *,
    ...
    vars_emitted = [v for v in vars_all if not _omit_from_configmap(v)]
    ...
    header: List[str] = [
        ...
        "#   python3 scripts/hasura/generate_hasura_config.py",
        "#",
        "# NOTE: Redis connection env vars (HASURA_GRAPHQL_*_REDIS_*) are configured",
        "# via Vault injection in manifests/hasura.yaml and are intentionally omitted",
        "# from this file/ConfigMap.",
        "#",
    ]
